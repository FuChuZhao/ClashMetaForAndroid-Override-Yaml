...
class FilesActivity : BaseActivity<FilesDesign>() {
    override suspend fun main() {
        ...
        importedDir.resolve(root).profileOverrideDir.mkdirs()
        pendingDir.resolve(root).profileOverrideDir.mkdirs()
    ...
    private suspend fun FilesDesign.fetch(client: FilesClient, stack: Stack<String>, root: String) {
        ...
        val files = if (stack.empty()) {
            ...
            val overrides = list.firstOrNull { it.isDirectory && it.id.endsWith("overrides") }

            if (config == null || config.size > 0) {
                list
            } else {
                listOfNotNull(config, overrides)
            }
        } else {
