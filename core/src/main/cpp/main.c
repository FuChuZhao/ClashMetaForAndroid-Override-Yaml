...
JNIEXPORT jstring JNICALL
Java_com_github_kr328_clash_core_bridge_Bridge_nativeApplyProfileOverrides(JNIEnv *env, jobject thiz,
                                                                          jstring path,
                                                                          jstring global_override_dir) {
    TRACE_METHOD();

    scoped_string _path = get_string(path);
    scoped_string _global_override_dir = get_string(global_override_dir);
    scoped_string response = applyProfileOverrides(_path, _global_override_dir);
