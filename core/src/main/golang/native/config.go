...
//export applyProfileOverrides
func applyProfileOverrides(path, globalOverrideDir C.c_string) *C.char {
	if err := config.ApplyProfileOverrides(C.GoString(path), C.GoString(globalOverrideDir)); err != nil {
