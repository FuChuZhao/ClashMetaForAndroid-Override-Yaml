...
import com.github.kr328.clash.service.util.profileOverrideDir
...
class Picker(private val context: Context) {
    suspend fun list(path: Path): List<Document> {
        ...
        if (path.scope == null) {
            return listOf(Path.Scope.Configuration, Path.Scope.Providers, Path.Scope.Overrides).map {
    ...
    suspend fun pick(path: Path, writable: Boolean): Document {
        ...
        if (path.relative == null) {
            if (path.scope == Path.Scope.Configuration) {
            ...
            } else if (path.scope == Path.Scope.Providers) {
                return FileDocument(
                    file = when {
                        pending != null -> context.pendingDir.resolve(pending.uuid.toString())
                        imported != null -> context.importedDir.resolve(imported.uuid.toString())
                ...
                val overrides = profileDir.profileOverrideDir
                overrides.mkdirs()
                return FileDocument(
                    file = overrides,
                    idOverride = Paths.OVERRIDES_ID,
                    nameOverride = context.getString(R.string.override_files),
                    flags = setOf(Flag.Virtual)
                )
            }
        }

        if (path.scope != Path.Scope.Providers && path.scope != Path.Scope.Overrides)
        ...
        val baseDir = when (path.scope) {
            Path.Scope.Providers -> profileDir.resolve("providers")
            Path.Scope.Overrides -> profileDir.profileOverrideDir
            else -> throw FileNotFoundException("invalid path")
        }

        return FileDocument(
            file = baseDir.resolve(path.relative.joinToString(separator = "/")),
